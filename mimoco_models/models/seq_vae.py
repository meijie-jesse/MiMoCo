...
"""Variational sequence policy backbone (multi-camera CNN + Transformer)."""
import torch
...
class SeqVAE(nn.Module):
    """Sequence actions from multi-camera images and proprio (CVAE-style)."""
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        """Args:
            backbones: per-camera CNN backbones (see backbone.py)
            transformer: decoder stack (see transformer.py)
            state_dim: proprio dimension
            num_queries: predicted action horizon (chunk length)
        """
        super().__init__()
...
class CNNMLP(nn.Module):
    def __init__(self, backbones, state_dim, camera_names):
        """Multi-camera CNN + MLP to actions."""
        super().__init__()
        self.camera_names = camera_names
