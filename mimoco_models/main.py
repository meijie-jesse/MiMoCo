...
import numpy as np
import torch
from .models import build_chunk_seq_model, build_CNNMLP_model

import IPython
e = IPython.embed

def get_args_parser():
    parser = argparse.ArgumentParser('MiMoCo sequence model', add_help=False)
...
def build_chunk_seq_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser('MiMoCo model training script', parents=[get_args_parser()])
    args = parser.parse_args()

    for k, v in args_override.items():
        setattr(args, k, v)

    model = build_chunk_seq_model(args)
    model.cuda()
